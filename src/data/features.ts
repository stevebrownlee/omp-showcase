...
export const features: Feature[] = [
  {
    ...
    explanation: "Managing API keys across multiple cloud providers is a chore. OMP solves this by letting you authenticate directly with your main subscriptions. Once logged in, every commercial and open-source model is at your fingertips. No more rotating keys, billing setup for four different developers, or environment variables getting leaked in git history.",
    ...
    terminalLines: [
      { type: 'input', text: 'omp login --all' },
      { type: 'info', text: '🔑 Authenticating with providers...' },
      { type: 'output', text: '✔ Connected to Anthropic (stevebrownlee)' },
      { type: 'output', text: '✔ Connected to OpenAI (stevebrownlee)' },
      { type: 'output', text: '✔ Connected to Google Gemini (stevebrownlee)' },
      { type: 'info', text: '🚀 24 models successfully unlocked.' }
    ],
    codeExample: {
  ...
  },
  {
    ...
    terminalLines: [
      { type: 'info', text: '🛡 Advisor mode initialized: Executor=Sonnet, Advisor=Gemini-Pro' },
      { type: 'output', text: 'Executor: Implementing pool manager in db.ts...' },
      { type: 'ticker', text: 'Advisor (Gemini-Pro): "Suggestion: Use an explicit connection timeout. The default pool might hang on connection drops."' }
    ]
  },
  {
    id: "planning-docs",
